    ...
    if 2 <= len(args):
        ...
        for arg in args:
            ...
            if os.path.isfile(arg): 
                ...
                rename_file(caption_data["filepath"], caption_data["caption"], caption_data["caption_ja"])
            # ディレクトリが存在する場合
            elif os.path.exists(arg):
                filelist = glob.glob(arg + "*.jpg")
                caption_data_list = list(map(lambda file: generate_caption(file), filelist))
                list(map(lambda cap: rename_file(cap["filepath"], cap["caption"], cap["caption_ja"]), caption_data_list))
            else:
